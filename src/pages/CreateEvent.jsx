...
import { Upload, Shield, CheckCircle, RefreshCw, AlertCircle, Mail, ExternalLink, Wallet, ChevronDown, X } from "lucide-react";
...
export default function CreateEventPage({ onCreated }) {
  ...
  const removeImage = () => { set("imagePreview", null); set("imageFile", null); setUploadErr(""); };

  // ── Upload image to Vercel Blob via /api/upload-image ─────────────────────
  const uploadImage = async (file) => {
    ...
    return data.url; // permanent public Vercel Blob CDN URL
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (form.imageFile) {
        setUploading(true);
        try {
          imageURI = await uploadImage(form.imageFile);
        } catch (err) {
  ...
  if (!wallet) return (
    <div style={{padding:"80px 24px",display:"flex",alignItems:"center",justifyContent:"center",minHeight:"80vh"}}>
      <div style={{textAlign:"center",maxWidth:400}}>
        <div style={{width:64,height:64,borderRadius:18,background:V.b50,border:"1px solid "+V.b100,
  ...
  if (done) return (
    <div style={{padding:"80px 24px",display:"flex",alignItems:"center",justifyContent:"center",minHeight:"80vh"}}>
      <div className="mdg card" style={{width:"100%",maxWidth:460,padding:38,textAlign:"center"}}>
        <div style={{width:60,height:60,borderRadius:17,background:"#F0FDF4",border:"1px solid #86EFAC",
          display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 18px"}}>
        ...
        <div style={{background:V.surface,border:"1px solid "+V.border,borderRadius:12,padding:"11px 14px",
          display:"flex",alignItems:"center",gap:10,marginBottom:18,textAlign:"left"}}>
          <div style={{flex:1,fontSize:12,fontFamily:"monospace",color:V.muted,overflow:"hidden",
            textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{done.shareUrl}</div>
          <button className="bg" style={{flexShrink:0,padding:"5px 10px",gap:4,fontSize:12,color:V.brand}}
        ...
        <button className="bp" onClick={() => navigate("/dashboard")}
          style={{width:"100%",justifyContent:"center",padding:13,marginBottom:10}}>
          View in Dashboard
        </button>
      </div>
    </div>
    ...
    <div style={{padding:"80px 24px 80px",maxWidth:680,margin:"0 auto"}}>
      ...
      {emailNotice && (
        <div className="mbd" onClick={e => e.target === e.currentTarget && setEN(false)}
          style={{position:"fixed",inset:0,background:"rgba(17,24,39,.4)",backdropFilter:"blur(8px)",
            display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,padding:20}}>
          <div className="mdg card" style={{width:"100%",maxWidth:400,padding:28}}>
            <div style={{width:46,height:46,borderRadius:13,background:"#FFFBEB",border:"1px solid #FCD34D",
      ...
      <div className="fu2 card" style={{padding:30}}>
        ...
        <div style={{marginBottom:28}}>
          <div style={{fontFamily:"Outfit",fontWeight:800,fontSize:13,color:V.muted,textTransform:"uppercase",
            letterSpacing:".08em",marginBottom:16,paddingBottom:10,borderBottom:"1px solid "+V.borderS}}>
            Event Information
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div>
              ...
              <textarea className="inp" placeholder="Describe the full event experience…" value={form.fullDescription}
                onChange={e => set("fullDescription", e.target.value)} style={{minHeight:90,resize:"vertical",lineHeight:1.6}}/>
            </div>
            <div>
              <Lbl>Category</Lbl>
              <div style={{position:"relative"}}>
                <select className="inp" value={form.category} onChange={e => set("category", e.target.value)}
                  style={{cursor:"pointer",appearance:"none",paddingRight:36}}>
                  {CATEGORIES.filter(c => c !== "All").map(c => <option key={c}>{c}</option>)}
                </select>
                <ChevronDown size={15} style={{position:"absolute",right:13,top:"50%",transform:"translateY(-50%)",
        ...
        <div style={{marginBottom:28}}>
          <div style={{fontFamily:"Outfit",fontWeight:800,fontSize:13,color:V.muted,textTransform:"uppercase",
            letterSpacing:".08em",marginBottom:16,paddingBottom:10,borderBottom:"1px solid "+V.borderS}}>
            Event Image <span style={{fontFamily:"DM Sans",fontSize:11,textTransform:"none",fontWeight:400,
              color:V.mutedL,letterSpacing:0}}>(optional)</span>
          </div>
          ...
          {form.imagePreview ? (
            ...
            <div style={{position:"relative",height:200,borderRadius:16,overflow:"hidden",
              ...
              <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.35)",display:"flex",
                alignItems:"flex-end",justifyContent:"space-between",padding:12}}>
                <span style={{color:"white",fontSize:12,fontFamily:"Outfit",fontWeight:600,
                  background:"rgba(0,0,0,.5)",padding:"4px 10px",borderRadius:8}}>
                  ✓ Image selected — will upload on create
                </span>
                <button onClick={removeImage}
                  style={{background:"rgba(239,68,68,.85)",border:"none",borderRadius:8,
                    padding:"5px 8px",cursor:"pointer",display:"flex",alignItems:"center",gap:4,
                    color:"white",fontSize:11,fontFamily:"Outfit",fontWeight:600}}>
                  <X size={11}/>Remove
                </button>
              </div>
            </div>
            ...
            <label htmlFor="imgup"
              style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
                gap:10,height:130,border:"2px dashed "+(uploadErr ? "#FCA5A5" : V.border),
                borderRadius:16,cursor:"pointer",background:V.surface,transition:"border-color .2s"}}
              onMouseEnter={e => { e.currentTarget.style.borderColor = V.brand; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = uploadErr ? "#FCA5A5" : V.border; }}>
          ...
          {/* Uploading indicator (shown during submit) */}
          {uploading && (
            <div style={{display:"flex",alignItems:"center",gap:8,marginTop:8,fontSize:13,color:V.brand}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div>
              ...
              <div style={{position:"relative"}}>
                ...
                <ChevronDown size={15} style={{position:"absolute",right:13,top:"50%",
        ...
        <div style={{marginBottom:28}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            <div>
              ...
              <div style={{position:"relative"}}>
                <input className="inp" placeholder="0  — leave empty for a free event" type="number"
                  min="0" step="0.001" value={form.ticketPrice} onChange={e => set("ticketPrice", e.target.value)}
                  style={{paddingRight: form.ticketPrice && form.ticketPrice !== "0" ? 136 : 16}}/>
                {form.ticketPrice && form.ticketPrice !== "0" && (
                  <span style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",
                    fontSize:12,color:V.muted,fontFamily:"Outfit",fontWeight:600}}>
            ...
            <div>
              ...
              <input className="inp" placeholder="e.g. 500" type="number" min="1"
                value={form.maxTickets} onChange={e => set("maxTickets", e.target.value)}/>
              <Err msg={errs.maxTickets}/>
            </div>

            {/* Email tickets toggle */}
            <div style={{background:V.surface,borderRadius:13,padding:"14px 16px",
              display:"flex",alignItems:"center",justifyContent:"space-between",opacity:isFree?1:.45}}>
              <div style={{flex:1,minWidth:0,paddingRight:14}}>
                <div style={{fontFamily:"Outfit",fontWeight:700,fontSize:14,color:V.text,marginBottom:3}}>
                  Accept Email Tickets
                </div>
                <div style={{fontSize:12,color:V.muted,lineHeight:1.55}}>
                  {isFree
                    ? "Let guests attend without a wallet — they receive an email confirmation."
              ...
              <button className="ttr" disabled={!isFree}
                onClick={() => {
                  if (!isFree) return;
                  if (form.acceptsOffchain) { set("acceptsOffchain", false); }
                  else { setEN(true); set("acceptsOffchain", true); }
                }}
        ...
        {/* Notice */}
        <div style={{background:V.b50,border:"1px solid "+V.b100,borderRadius:12,padding:"11px 14px",
          display:"flex",gap:9,marginBottom:22}}>
        ...
        {txErr && (
          <div style={{background:"#FEF2F2",border:"1px solid #FCA5A5",borderRadius:12,
            padding:"12px 14px",marginBottom:18,display:"flex",alignItems:"center",gap:9,
            fontSize:13,color:"#DC2626"}}>
            <AlertCircle size={14}/>{txErr}
          </div>
        )}

        <button className="bp" onClick={submit} disabled={busy || !form.name}
          style={{width:"100%",justifyContent:"center",padding:14,borderRadius:14,fontSize:15}}>
          {uploading
            ? <><RefreshCw size={15} className="spin"/>Uploading image…</>
            : busy
              ? <><RefreshCw size={15} className="spin"/>Creating event on 0G…</>
