...
export default function ScanModal({ events = [], onClose }) {
  ...
  useEffect(() => () => { mountedRef.current = false; stopCamera(); }, []);
  ...
  const decodeFrame = useCallback(() => {
    ...
    if (!video || !canvas || video.readyState < 2 || processing) return;
  ...
  const handleQRFound = useCallback(async (raw) => {
    ...
    if (!parsed) {
      setResultErr(`Unrecognised QR code.\nGot: "${raw.slice(0, 60)}${raw.length > 60 ? "…" : ""}"`);
      setResult({ ok: false });
      setScanState("done");
      setProcessing(false);
      return;
    ...
    if (epoch < now - MAX_EPOCH_AGE) {
      setResultErr(`QR expired (epoch ${epoch}, now ${now}). Ask the attendee to re-reveal their ticket.`);
      setResult({ ok: false });
      setScanState("done");
      setProcessing(false);
      return;
    ...
    if (sel && eventId !== sel.id) {
      setResultErr(`Ticket is for event #${eventId}, not "${sel.name}" (#${sel.id}).`);
      setResult({ ok: false });
      setScanState("done");
      setProcessing(false);
      return;
            ...
            <>
              ...
              {sel && (
                <div>
                  ...
                  <div style={{borderRadius:16,overflow:"hidden",background:"#0a0a14",position:"relative",aspectRatio:"4/3",marginBottom:14}}>
                    ...
                    {camState === "error" && (
                      <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14,padding:24,textAlign:"center"}}>
                        ...
                        <button className="bp" onClick={startCamera} style={{borderRadius:11,padding:"9px 20px",fontSize:13}}>
                          Try Again
                        </button>
                      </div>
                    )}
                    ...
                    {camState === "active" && (
                      <div style={{position:"absolute",inset:0,pointerEvents:"none"}}>
                        ...
                        <div style={{position:"absolute",bottom:14,left:"50%",transform:"translateX(-50%)",background:"rgba(0,0,0,.7)",backdropFilter:"blur(8px)",borderRadius:20,padding:"7px 18px",display:"flex",alignItems:"center",gap:8,whiteSpace:"nowrap"}}>
                          {scanState === "scanning" && (
                            <><div style={{width:8,height:8,borderRadius:"50%",background:"#00C48A",animation:"pulse 1.2s ease-in-out infinite"}}/><span style={{color:"white",fontSize:12,fontFamily:"Outfit",fontWeight:600}}>Scanning…</span></>
                          )}
                          {scanState === "decoding" && (
                            <><RefreshCw size={13} color="white" className="spin"/><span style={{color:"white",fontSize:12,fontFamily:"Outfit",fontWeight:600}}>Decoding QR…</span></>
                          )}
                          {scanState === "verifying" && (
                            <><RefreshCw size={13} color="#A78BFA" className="spin"/><span style={{color:"#A78BFA",fontSize:12,fontFamily:"Outfit",fontWeight:600}}>Verifying on-chain…</span></>
                          )}
                        </div>
                      </div>
                    )}
                  ...
                  {scanState === "done" && (
                    <button className="bp" onClick={resetScan}
                      style={{width:"100%",justifyContent:"center",padding:12,borderRadius:13}}>
